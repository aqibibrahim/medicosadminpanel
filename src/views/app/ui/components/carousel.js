      ...
      <Card className="flex-row">
          ...
          {badges &&
            badges.map((b, index) => {
              return (
                <span
                  key={index}
                  className={`badge badge-pill badge-${b.color} position-absolute ${
                    index === 0 ? "badge-top-left" : `badge-top-left-${index + 1}`
                  }`}
                >
                  {b.title}
                </span>
        ...
        <div className="w-50">
          <CardBody>
            ...
            <footer>
              <p className="text-muted text-small mb-0 font-weight-light">{detail}</p>
            </footer>
          </CardBody>
        </div>
      </Card>
...
const SingleCarouselItem = ({ title, img, category, detail, badges }) => {
  return (
    <div className="glide-item">
      <Card className="flex-row">
        <img className="list-thumbnail responsive border-0 card-img-left" src={img} alt={title} />
              ...
              {badges &&
                badges.map((b, index) => {
                  return (
                    <span
                      key={index}
                      className={`badge badge-pill badge-${b.color} ${index < badges.length && "mr-1"}`}
                    >
                      {b.title}
                    </span>
                  );
